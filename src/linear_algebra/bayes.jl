...
export LinearFunctions, FreeLinearFunctions, LinearRelations,
  ...
  LinearOpDom, LinearOperator, StochMapDom, OpenBayesNet, FreeBayesFunctions,
...
"""
Doctrine of *BayesFunctions*

"""
# @theory MonoidalCategoryWithDiagonals(Ob, Hom) => BayesFunctions(Ob, Hom) begin
#   # Copying and deleting maps.
#   mcopy(A::Ob)::(A → (A ⊗ A))
#   @op (Δ) := mcopy
#   delete(A::Ob)::(A → mzero())
...
#         end
#     end
    StochMapDom(len(OpenBayesNet.dom))
  end
...
#         end
#     end
    StochMapDom(len(OpenBayesNet.codom))
  end

  compose(f::OpenBayesNet, g::OpenBayesNet) = begin
    dom = f.dom
    codom = [len(f.dom)+ elem for elem in g.codom]
    ...
    for (key,value) in g.Net.name_to_index
      if value <= len(g.dom)
        ...
        combined_dict[key] = value + len(f.dom)
      end
    end
